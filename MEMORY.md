...
## 2026-09-18 — 데모 초기화 + 샘플 바로 올리기 (16차)

- `POST /demo/reset`(데모 계정만) + `me.isDemo` → MY 「데모 초기화」 카드(데모에서만 노출). 초기화 후 빈 상태에서 시연 시작
- 서류 탭 샘플: 유형별 필터 + [바로 올리기](fetch→File→기존 업로드 경로). 로컬 풀플로우 검증, e2e 59/59
- 사용자 확인: 실제 크롬에서 카카오 주소검색으로 케이스 생성됨 (내장 브라우저에서만 위젯 검색 불가)
