...
- [ ] `/analysis`·문서 뷰어·홈 판정을 실 API 결과에 연결 — 현재 `D.ANALYSIS`/`D.DOCTEXT` 샘플 사용. 예시 안내는 실데이터 연결·검증 후 제거
...
2026-09-17 — 업로드 서버 오류의 데모 성공 폴백 제거, 폴링 취소·시간 제한·실패 안내 추가.
실 업로드는 서버 서류 상태를 갱신하며 고정 발급일을 덮어쓰지 않음.
종합 판정에 unknown을 반영하고 샘플 분석/본문을 명시. `npm test`로 회귀 검증.
