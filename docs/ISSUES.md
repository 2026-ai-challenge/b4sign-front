...
- 🔴 **막힌 지점: ErrorCode 9910025 "정액제 사용 기간이 아닙니다"** — 포인트 10,000p가 있어도
  이 API(부동산 등기물건 주소검색, ApiCode A04015101001)는 **대시보드에서 사용 신청/요금제
  활성화**가 필요한 상태. → tilko.net 대시보드에서 등기소 API 사용 신청 (안 보이면 틸코
  고객센터에 "A04015 포인트제 사용 신청" 문의). 활성화되면 코드 수정 없이 동작 예상.
