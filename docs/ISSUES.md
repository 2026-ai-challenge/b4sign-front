...
## 6. 프론트 케이스 동적화 — ✅ 해결 (2026-09-18)

(원래 문제) 백엔드에 케이스 CRUD가 있었지만 프론트는 12개 파일에서 `D.CASES`(데모 3건)를
정적으로 참조했고, 케이스 생성 화면도 "샘플로 이동" 데모 동작이었다.

조치:
- `store.cases`가 **bootstrap `cases` 전체**를 상태로 보유 (데모 id면 D.CASES 메타 병합, 그 외 id도 렌더).
  `currentCase` 파생값, `addCase(payload)`(POST /cases → 서류·할일 로드 → 선택), `removeCase(cid, hard)`.
- 전 화면 `D.CASES.find(...)` → `currentCase` (분석·뷰어·체크리스트·할일·상담·등기감시·대시보드·서류).
  `buildDocList(cur, docs)`가 케이스 객체를 받음. 케이스 없음/미선택은 `NoCase` 빈 상태 컴포넌트.
- `cases/new` → 실제 `addCase` 후 `/documents`로 이동. 알림 로컬 폴백도 `cases`를 순회.
- 선택 케이스는 `localStorage zipsalpi_case`에 저장하되 **초깃값은 "c1", 복원은 useEffect**(하이드레이션 불일치 방지).
- 로컬 검증: 신규 케이스(월세) 생성 → 대시보드 헤더/드롭다운, 서류(미제출 4종 + 샘플), 할일(기본 12건·기한 계산), 상담 인사 모두 렌더.

남은 것: 글로벌 하이드레이션 경고(React #418)는 **이 작업 이전부터** /login 포함 전 페이지에 존재 — 별도 추적.
