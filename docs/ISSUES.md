...
- ✅ **주소검색 API 실검증 완료 (2026-09-17)**: 키 타입은 **"일반용"이 포인트 종량제**
  (정액제 키는 9910025로 거절). 바디 규칙 확정 — **개인정보가 없는 필드는 전부 평문**
  (암호화하면 7701001/형식 오류). `POST /registry/search`가 실제 부동산 고유번호(pin) 목록을
  반환하는 것까지 백엔드 경유로 확인 (포인트 정상 차감, 잔액 9,920).
  발급(RealtyRegistry)은 Auth(계정·캐시)만 암호화로 두고 옵션 필드 평문 적용 — 캐시 비밀번호
  확보 후 1건 실검증만 남음.
