...
- 🔴 **막힌 지점 원인 확정 (2026-09-17, MY PAGE 스크린샷)**: 발급된 API KEY의 **KEY TYPE이
  "정액제"** — 포인트 10,000p가 있어도 정액제 키로는 포인트 차감 호출이 안 된다(9910025).
  → **MY PAGE > API KEY > [+추가]에서 KEY TYPE을 포인트제(종량제)로 새 키 발급** 후
  `.env`의 TILKO_API_KEY 교체. (추가가 안 되면 틸코 고객센터/카톡 특상담에 "포인트제 키 발급" 요청)
