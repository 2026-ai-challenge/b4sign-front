...
## 분석 화면의 실데이터 연결 — 후속 필수 (2026-09-17 코드 확인)

- 홈·분석 화면은 `D.ANALYSIS`, 문서 뷰어는 `D.DOCTEXT`의 예시 데이터를 읽는다.
  `/bootstrap` 성공이나 업로드 job `done`만으로 해당 화면이 실데이터로 바뀌지 않는다.
- 이번 수정은 샘플 안내를 추가하고, 실제 업로드/폴링 오류를 데모 성공으로 바꾸던 경로를 제거한다.
  서버 완료 후 서류 상태 갱신에 성공해야 완료를 안내한다. 모달 닫기/페이지 이탈/케이스 전환 시
  프론트 요청을 취소하지만 서버에서 이미 시작된 작업을 취소하는 기능은 아니다.
- API를 설정하지 않은 데모에서는 파일 전송·분석을 하지 않는다는 안내 후 기존 예시 흐름을 제공한다.
- 후속: 명세의 `GET /cases/{caseId}/analysis`, `GET /documents/{documentId}`를 실제 상태에 연결하고
  문서 ID·분석 시점·케이스 매핑을 검증한 뒤 예시 안내를 제거한다.
- `ok`/`stale`는 서류 상태이므로 그 값만으로 OCR 완료나 외부 API 검증 완료를 단정하지 않는다.
